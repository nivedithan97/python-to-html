...
def getthehtml(NoL, MlL, MaxV, MinV, MaxCmt, NoD, NoS, NoN, NoR, code, filename):
    ...
    #convert from "filename.py" to "filename.html"
            ...
            <body>
                ...
                <div id="stats">
                    ...
                    <ul>
                        ...
                        <li> Maximum variable length =  {str(MaxV)} </li>
                        <li> Minimum variable length =  {str(MinV)} </li>
                        ...
                        <li> Number of repeated constant =  {str(NoR)} </li>
                    </ul>
                </div>
                <div id="code">
                    <h2>Python code</h2>
...
def get_contents(f, fl, lst, filename):
    # f = contents, fl = contentslines, tk = tokens, fn = filenames
...
def countMaxCmmtLine(lst_tk):
    ...
    for token in lst_tk:
        ...
        if token[0] == 60:
            counter += 1
    #print('countMaxCmmtLine is', counter)
    return counter
...
def countNumOfDefs(lst_tk):
    ...
    for token in lst_tk:
        if token[0] == 1:
            ...
            if token[1] == 'def':
                counter += 1
    #print('countNumOfDefs is', counter)
    return counter

"""
This function is to cont the number of strings in the original .py file
...
def countNumOfStrs(lst_tk):
    ...
    for token in lst_tk:
        ...
        if token[0] == 3:
            counter += 1
    #print('countNumOfStrs is', counter)
    return counter

"""
This function is to cont the mumber of numbers that appeared in the original file and is not in comments
...
def countNumOfNums(lst_tk):
    ...
    for token in lst_tk:
        ...
        if token[0] == 2:
            counter += 1
    #print('countNumOfNums is', counter)
    return counter

"""
This function is to cont the mumber of constants that appeared more than once in the original file
...
def color_token(token):
    """
    Provides the token type and the token value for a given token and
    formats it accordingly for HTML syntax coloring
...
if __name__ == '__main__':
    ...
    # filename ='ut1.py'#added for testing
